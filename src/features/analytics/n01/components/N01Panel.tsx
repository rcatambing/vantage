...
import { useN01 } from "../hooks/useN01";
...
export default function N01Panel({ campaignId, onRemove }: Props) {
  const { data, loading, error, refetch } = useN01(campaignId);

  return (
    <Panel
      id="n01"
      name="N01 — Signal Freshness Index"
      size="medium"
      onRemove={onRemove}
    ...
    >
      ...
      {!loading && !error && !campaignId && (
        <div style={{ padding: 16, textAlign: "center", color: GRAY_50 }}>
          Select a campaign to view signal freshness data.
        </div>
      )}
      {!loading && !error && data && (
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: GRAY_50, marginTop: 4 }}>
                Signal Freshness Index
              </div>
            </div>
            <div style={{ fontSize: 11, color: GRAY_50 }}>
              <div>{data.total_signals} signals</div>
              <div>{data.half_life_days}d half-life</div>
            </div>
          </div>
              ...
              <HTMLTable compact striped style={{ width: "100%" }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: "right" }}>SFI</th>
                  </tr>
                </thead>
                <tbody>
                  {data.district_breakdown.map((d) => (
                    <tr key={d.district_id}>
                      ...
                      <td style={{ textAlign: "right", color: BLUE_40 }}>{d.sfi_display}</td>
                    </tr>
                  ))}
                </tbody>
              </HTMLTable>
