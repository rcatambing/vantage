import type { M33DistrictEntry } from "../types";
...
export default function DistrictUtilizationBar({ data }: Props) {
  if (!data.length) return null;

  const maxTotal = Math.max(...data.map((d) => d.total_assigned_count), 1);
  const chartW = W - PAD_LEFT - PAD_RIGHT;
  const totalH = PAD_TOP + data.length * ROW_H;

  return (
    <svg viewBox={`0 0 ${W} ${totalH}`} width="100%" style={{ display: "block" }} aria-label="Area assignment utilization by district">
      {data.map((entry, i) => {
        const y = PAD_TOP + i * ROW_H;
        const activeW = Math.max(2, (entry.active_staff_count / maxTotal) * chartW);
        const inactiveW = Math.max(2, ((entry.total_assigned_count - entry.active_staff_count) / maxTotal) * chartW);
        ...
        return (
          <g key={entry.district_id}>
            ...
            <text x={PAD_LEFT + chartW + 4} y={y + 16} fontSize={11} fill={GRAY_50}>
              {entry.active_staff_count}/{entry.total_assigned_count}
            </text>
          </g>
        );
      })}
